...
export class DetallePlatoService {
  url = environment.urlApi + 'DetallePlatos/';
  textCarga = "Detalle plato ";

  constructor(private httpClient: HttpClient, private textShared:TextShared) {}

  public registro(user:DetallePlato): Promise<HttpResponse<any>>{
    this.textShared.textCarga = "Insertando " + this.textCarga;
    return new Promise((resolve, reject)=>{
      this.httpClient
      .post<any>(this.url + `registrar`,user,{ observe: 'response' })
      .subscribe(response=>{
        resolve(response);
      },err=>reject(err));
    });
  }
  public actualizar(user:DetallePlato): Promise<HttpResponse<any>>{
