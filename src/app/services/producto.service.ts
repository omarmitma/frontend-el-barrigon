...
export class ProductoService {
  ...
  public registro(user:Producto): Promise<HttpResponse<any>>{
  ...
  public actualizar(user:Producto): Promise<HttpResponse<any>>{
  ...
  public buscarAll_Inner_by_id(idProducto:number): Promise<HttpResponse<any>>{
    this.textShared.textCarga = "Buscando " + this.textCarga;
    return new Promise((resolve, reject)=>{
      this.httpClient
      .get<any>(this.url + `buscarAll_Inner_by_id?idProducto=${idProducto}`,{ observe: 'response' })
      .subscribe(response=>{
        resolve(response);
      },err=>reject(err));
    });
