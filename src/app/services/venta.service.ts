...
export class VentaService {
  url = environment.urlApi + 'Ventas/';
  textCarga = "Venta ";

  constructor(private httpClient: HttpClient, private textShared:TextShared) {}

  public registro(user:VentaService): Promise<HttpResponse<any>>{
  ...
  public actualizar(user:VentaService): Promise<HttpResponse<any>>{
  ...
  //   this.textShared.textCarga = "BuscandoALL " + this.textCarga;
  //   return new Promise((resolve, reject)=>{
  //     this.httpClient
  //     .put<any>(this.url + `buscarAll`,user,{ observe: 'response' })
  //     .subscribe(response=>{
  //       resolve(response);
  //     },err=>reject(err));
  //   });
