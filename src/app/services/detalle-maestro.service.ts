...
import { DetalleMaestro } from '../entitys/detalleMaestro';

@Injectable({
  providedIn: 'root'
})
export class DetalleMaestroService {
  url = environment.urlApi + 'DetalleMaestros/';
  textCarga = "Detalle Maestro ";

  constructor(private httpClient: HttpClient, private textShared:TextShared) {}

  public registro(user:DetalleMaestro): Promise<HttpResponse<any>>{
  ...
  public actualizar(user:DetalleMaestro): Promise<HttpResponse<any>>{
